...
void *poller(void *thread_args)
{
    ...
    while (1) {
        ...
        if (set.verbose >= DEVELOP) {
            ...
            syslog(LOG_DEBUG,
                   "Thread [%d] done waiting, received go (work cnt: %d)\n", worker->index, crew->work_count);
        }

        if (current != NULL) {
            if (set.verbose >= HIGH) {
                printf
                    ("Thread [%d] processing %s %s (%d work units remain in queue)\n",
        ...
        if (status == STAT_SUCCESS && response->errstat == SNMP_ERR_NOERROR) {
            ...
            case ASN_COUNTER64:
                ...
                bits = 64;
                strcpy(type, "float"); /* because counter will be converted to a rate */
                break;
            case ASN_COUNTER:
                if (set.verbose >= DEBUG)
            ...
            case ASN_INTEGER:
                ...
                snprintf(buf_val, sizeof(buf_val), "%llu", (unsigned long) *(vars->val.integer));
                strcpy(type, "uint32");
                break;
            case ASN_GAUGE:
                if (set.verbose >= DEBUG)
            ...
            case ASN_TIMETICKS:
                ...
                snprintf(buf_val, sizeof(buf_val), "%llu", (unsigned long) *(vars->val.integer) / 100); /* TimeTicks are hundredths of a second */
                strcpy(type, "uint32");
                break;
            case ASN_OPAQUE:
                if (set.verbose >= DEBUG)
            ...
            case ASN_OCTET_STR:
                if (set.verbose >= DEBUG)
                    printf("String result: (%s@%s) %s\n", session.peername, storedoid, result_string);
                snprintf(buf_val, sizeof(buf_val), "%s", (char *) vars->val.string);
                strcpy(type, "string");
                break;
            case ASN_OPAQUE_FLOAT:
                if (set.verbose >= DEBUG)
                    printf("Opaque Float result: (%s@%s) %s\n", session.peername, storedoid, result_string);
                snprintf(buf_val, sizeof(buf_val), "%.2f", *(vars->val.floatVal));
                strcpy(type, "float");
                break;
            default:
                if (set.verbose >= DEBUG)
            ...
            if (bits > 0) {
                if (result < last_value) {
                    ...
                    if (set.verbose >= LOW) {
                        printf
                            ("*** Counter Wrap (%s@%s) [poll: %llu][last: %llu][insert: %llu]\n",
                ...
                if (insert_val > entry->maxspeed || result < 0) {
                    if (set.verbose >= LOW) {
                        printf
                            ("*** Out of Range (%s@%s) [insert_val: %llu] [oor: %lld]\n",
                ...
                rate = (float) insert_val / set.interval;
                snprintf(buf_val, sizeof(buf_val), "%.2f", rate);
            }

            if (!(set.goff)) {
                PT_MUTEX_LOCK(&crew->mutex);

                // Get IP address of SNMP device for host spoofing
                ...
                if (getaddrinfo(session.peername, NULL, &hints, &res)
                    != 0)
                ...
                session.peername = strtok( session.peername, ".\0" );
                snprintf(spoof, sizeof(spoof), "%s:%s", ipstr, session.peername);

                /* set units to empty string if none supplied */
                if (!strcmp(entry->units, "\"\""))
                    snprintf(entry->units, sizeof(entry->units), "");
                /* any non-zero slope value results in slope of "both" */
                slope = (slope ? GANGLIA_SLOPE_BOTH : GANGLIA_SLOPE_ZERO);

                if (set.verbose >= DEBUG)
                    printf
                        ("SEND: gmetric --spoof %s:%s --name %s --value %s --type %s --units %s --slope %s --tmax %d --dmax %d\n",
                         ipstr, session.peername, entry->table,
                         buf_val, type, entry->units, 
                         (slope ? "both" : "zero"),
                         set.interval, entry->iid);

                if (set.verbose >= DEBUG)
                    printf
                        (" --group %s --description %s --title %s\n",
                ...
                rval = Ganglia_metric_set(gmetric, entry->table, buf_val, type, entry->units, slope, set.interval, entry->iid);      // FIXME -- is dmax ignored?
                ...
                if (rval) {
                    printf
                        ("*** Ganglia Error: There was an error sending to %d of the send channels.\n", rval);
                    syslog(LOG_ERR,
                           "*** Ganglia Error: There was an error sending to %d of the send channels.", rval);
                }
