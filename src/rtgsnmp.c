...
void *poller(void *thread_args)
{
	...
	if (current != NULL) {
		...
		printf
		    ("Thread [%d] processing %s %s (%d work units remain in queue)\n",
		     worker->index, current->host, current->objoid,
		     crew->work_count);
		syslog(LOG_NOTICE,
		       "Thread [%d] processing %s %s (%d work units remain in queue)\n",
		       worker->index, current->host, current->objoid,
	...
	if (set.verbose >= DEVELOP) {
	    printf("Thread [%d] unlocking (done grabbing current)\n",
		   worker->index);
	    syslog(LOG_DEBUG,
		   "Thread [%d] unlocking (done grabbing current)\n",
		   worker->index);
	}
	...
	if (status == STAT_SUCCESS
		...
		if (set.verbose >= DEBUG)
		    printf("String result: (%s@%s) %s\n", session.peername,
			   storedoid, result_string);
		snprintf(result_string, BUFSIZE, "%s",
		...
		if (result != last_value) {
		    ...
		    if (set.verbose >= HIGH)
			printf
			    ("Thread [%d]: Gauge change from %lld to %lld\n",
		...
		if (set.verbose >= LOW) {
			...
			("*** Counter Wrap (%s@%s) [poll: %llu][last: %llu][insert: %llu]\n",
			 session.peername, storedoid, result, last_value,
			 insert_val);
		}
		...
		if (set.verbose >= LOW) {
			...
			("*** Out of Range (%s@%s) [insert_val: %llu] [oor: %lld]\n",
			 session.peername, storedoid, insert_val,
			 entry->maxspeed);
		    syslog(LOG_WARNING,
			   "*** Out of Range (%s@%s) [insert_val: %llu] [oor: %lld]\n",
			   session.peername, storedoid, insert_val,
		...
		if (getaddrinfo(session.peername, NULL, &hints, &res) != 0)
		    syslog(LOG_ERR,
			   "*** Host Resolution Error: getaddrinfo() failed %s.",
			   session.peername);
		...
		char spoof[BUFSIZE];
		snprintf(spoof, sizeof(spoof), "%s:%s", ipstr,
			 session.peername);
		...
		if (set.verbose >= DEBUG)
		    printf
			("SEND: gmetric --spoof %s:%s --name %s --value %s --type %s --units %s --slope both --tmax %d --dmax %d\n",
			 ipstr, session.peername, entry->table, buf_val,
			 (is_string == 0 ? "uint32" : "string"),
			 entry->units, set.interval, entry->iid);
		...
		if (!gmetric) {
		    printf
			("*** Ganglia Error: Unable to allocate gmetric structure. Exiting.\n");
		    syslog(LOG_ERR,
			   "*** Ganglia Error: Unable to allocate gmetric structure. Exiting.");
		    closelog();
		    exit(1);
		...
		rval = Ganglia_metric_set(gmetric, entry->table, buf_val, (is_string == 0 ? "uint32" : "string"), entry->units, (bits ? GANGLIA_SLOPE_BOTH : GANGLIA_SLOPE_ZERO), set.interval, entry->iid);	// FIXME -- is dmax ignored?
	...
	if (set.verbose >= DEVELOP) {
	    printf("Thread [%d] unlocking (update work_count)\n",
		   worker->index);
	    syslog(LOG_DEBUG,
		   "Thread [%d] unlocking (update work_count)\n",
		   worker->index);
	}
