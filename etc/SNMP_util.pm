...
sub snmpwalk_flg ($$@) {
  my($host, $hash_sub, @vars) = @_;
  my(@enoid, $var, $response, $bindings, $binding);
  my($value, $upoid, $oid, $rval, @retvals, @retvaltmprefs);
  my($got, @nnoid, $noid, $ok, $ix, @avars);
  my $session;
  my(%soid);
  my(%done, %rethash);
  my $return_array_refs = 0;
	
  # IlvJa
  # Is there any options supplied for snmpwalk alone?

  if(ref $vars[0] eq 'HASH'
     and exists $vars[0]->{'return_array_refs'})
  {
      $return_array_refs = $vars[0]->{'return_array_refs'};
      # Now delete the option as it's not intended for snmpopen.
      delete $vars[0]->{'return_array_refs'};
      shift @vars unless scalar (%{$vars[0]});
  }
  ...
  # IlvJa
  #
  # Create temporary array of refs to retun vals.
    ...
    : $session->getnext_request_response(@nnoid))
  {
    ...
    while ($bindings) {
	  ...
	  $avars[$ix] = "";
	  $nnoid[$ix] = "";
	  $retvaltmprefs[$ix] = undef if $return_array_refs;
      }
      if ($ok) {
    ...
    # the walk related to the nth variable in the last requet has been
