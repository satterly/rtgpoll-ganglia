...
void walk_target_hash()
{
	...
	while (p) {
	    printf("[%d/%d]: %s %s %d %s %s %s %d %s %s %s\n", targets, i,
		   p->host, p->objoid, p->bits, p->community, p->table,
		   p->units, p->iid, p->group, p->title, p->descr);
	    targets++;
	    p = p->next;
	}
    }
...
int compare_targets(target_t * t1, target_t * t2)
{
	...
	!strcmp(t1->community, t2->community) && (t1->iid == t2->iid))
	return TRUE;
    return FALSE;
}
...
int hash_target_file(char *file)
{
	...
	if (!feof(fp) && buffer[0] != '#' && buffer[0] != ' '
	    ...
	    sscanf(buffer,
		   ...
		   new->table, new->units, &(new->iid), new->group,
		   new->title, new->descr);
