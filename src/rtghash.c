	...
	for (i=0;i<HASHSIZE;i++) {
		...
		while (p) {
			printf("[%d/%d]: %s %s %d %s %s %s %d %s %s %s\n", targets, i, p->host,
				p->objoid, p->bits, p->community, p->table, p->units,
				p->iid, p->group, p->title, p->descr);
			targets++;
			p = p->next;
		}
	}
		...
		if (compare_targets(entry, list)) {
			if (set.verbose >= HIGH) {
				printf("Found existing %s %s %s %s %d\n", list->host,
					list->objoid, list->table, list->units, list->iid,
                                        list->group, list->title, list->descr);
			}
			return list;
		}
		list = list->next;
...
int hash_target_file(char *file) {
	...
	while (!feof(fp)) {
		...
		if (!feof(fp) && buffer[0] != '#' && buffer[0] != ' ' && buffer[0] != '\n') {
			...
			sscanf(buffer, "%64s %128s %hu %64s %64s %64s %d \"%64[^\"]\" \"%64[^\"]\" \"%128[^\"]\"",
			       new->host, new->objoid, &(new->bits),
			       new->community, new->table, new->units,
			       &(new->iid), new->group, new->title, new->descr);
